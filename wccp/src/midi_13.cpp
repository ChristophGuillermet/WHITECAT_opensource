...
ol::Rect ChangeCh(ol::Vec2D(xrep+110,yrep-30),ol::Vec2D(80,20));
...
ol::Rect BackDeroule(ol::Vec2D(xrep,yrep),ol::Vec2D(190,210));
BackDeroule.SetRoundness(15);
BackDeroule.Draw(CouleurFond.WithAlpha(0.3));

for (int y=0;y<10;y++)
{
ol::Rect OverMidi(ol::Vec2D(xrep+5,(yrep+10+(y*20)-10)),ol::Vec2D(140,15));
...
//////////////////UP DOWN LINE save/////////////////////
ol::Circle LineUp(ol::Vec2D(xrep+170,yrep+25),12);
LineUp.Draw(CouleurFond);
ol::Circle LineDown(ol::Vec2D(xrep+170,yrep+185),12);
...
for (int i=1;i<3;i++)
{
     ol::Rect MidiChooseTypeBox( ol::Vec2D(cfg_midiX+20,cfg_midiY+60+(i*30)), ol::Vec2D( 60,15));
...
/////////////////////////////////////////////////////////////////////////
///////////MANUAL INPUT
  ol::Rect MidiGlob( ol::Vec2D(cfg_midiX+550,cfg_midiY+120), ol::Vec2D( 270,120));
  MidiGlob.SetRoundness(7.5);
  MidiGlob.SetLineWidth(epaisseur_ligne_fader);
  MidiGlob.Draw(CouleurFond.WithAlpha(0.2));

  ol::Rect MidiChBox( ol::Vec2D(cfg_midiX+630,cfg_midiY+150), ol::Vec2D( 70,30));
  MidiChBox.SetRoundness(7.5);
  MidiChBox.SetLineWidth(epaisseur_ligne_fader);


  ol::Rect MidiPitchBox( ol::Vec2D(cfg_midiX+710,cfg_midiY+150), ol::Vec2D( 100,30));
  MidiPitchBox.SetRoundness(7.5);
  MidiPitchBox.SetLineWidth(epaisseur_ligne_fader);


  ol::Rect MidiTypeBox( ol::Vec2D(cfg_midiX+710,cfg_midiY+190), ol::Vec2D( 100,30));
...
for (int i=1;i<3;i++)
{
     ol::Rect MidiModeBox( ol::Vec2D(cfg_midiX+560,cfg_midiY+70+(i*30)), ol::Vec2D( 60,15));
...
///////////MIDI NUMERIC TOGGLE MODE
     ...
     ol::Rect MidiTogBox( ol::Vec2D(cfg_midiX+560,cfg_midiY+50), ol::Vec2D( 60,25));
...
ol::Rect LaunchCol(ol::Vec2D(cfg_midiX+40,cfg_midiY+20+(pm*25)),ol::Vec2D(40,20));
...
ol::Rect LaunchRows(ol::Vec2D(cfg_midiX+40,cfg_midiY+190),ol::Vec2D(40,20));
...
{
ol::Rect MidiOption(ol::Vec2D(cfg_midiX+20,cfg_midiY+45+(pm*30)),ol::Vec2D(55,20));
...
int midipage_devices(int cfg_midiX,int cfg_midiY, int largeurCFGmidi,int hauteurCFGmidi)
{
    ...
    ol::Rect MidiInDev( ol::Vec2D(cfg_midiX+15,cfg_midiY+40), ol::Vec2D( 400,240));
    MidiInDev.SetRoundness(5);
    MidiInDev.Draw(CouleurFond);


    ol::Rect MidiOutDev( ol::Vec2D(cfg_midiX+420,cfg_midiY+40), ol::Vec2D( 400,240));
    ...
    if (i<=8)
    {
     ...
     ol::Rect MidiDeviceBox( ol::Vec2D(cfg_midiX+20,cfg_midiY+15+(i*30)), ol::Vec2D( 190,25));
    ...
    {
     ol::Rect MidiDeviceBox( ol::Vec2D(cfg_midiX+220,cfg_midiY+15+((i-8)*30)), ol::Vec2D( 190,25));
     MidiDeviceBox.SetRoundness(7.5);
     MidiDeviceBox.SetLineWidth(epaisseur_ligne_fader);
     if( window_focus_id==920)
     {
  ...
  // MIDI OUTPUT
    ...
    if(j<=8)
    {
     ol::Rect MidiDeviceBox( ol::Vec2D(cfg_midiX+420,cfg_midiY+15+(j*30)), ol::Vec2D( 190,25));
     MidiDeviceBox.SetRoundness(7.5);
     MidiDeviceBox.SetLineWidth(epaisseur_ligne_fader);
     if( window_focus_id==920)
     {
    ...
    {
     ol::Rect MidiDeviceBox( ol::Vec2D(cfg_midiX+620,cfg_midiY+15+((j-8)*30)), ol::Vec2D( 190,25));
     MidiDeviceBox.SetRoundness(7.5);
     MidiDeviceBox.SetLineWidth(epaisseur_ligne_fader);
     if( window_focus_id==920)
     {
    ...
    ol::Rect LabelIn(ol::Vec2D(cfg_midiX+372,cfg_midiY+30),ol::Vec2D(40,20));
    LabelIn.Draw(CouleurBleuProcedure);
    petitchiffre.Print("INPUT",cfg_midiX+375,cfg_midiY+43);

    ol::Rect LabelOut(ol::Vec2D(cfg_midiX+768,cfg_midiY+30),ol::Vec2D(50,20));
    LabelOut.Draw(CouleurBlind.WithAlpha(0.5));
    petitchiffre.Print("OUTPUT",cfg_midiX+770,cfg_midiY+43);

 ///RESCAN

    ol::Rect MidiRescanDriver( ol::Vec2D(cfg_midiX+720,cfg_midiY), ol::Vec2D( 70,25));
...
{
ol::Rect MidiMenu( ol::Vec2D(cfg_midiX+20+(pp*150),cfg_midiY+1), ol::Vec2D( 140,35));
