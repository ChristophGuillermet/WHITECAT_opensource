...
int RetourInfos(int x_info,int y_info)
{
    ...
    ol::Rect VisuHue(ol::Vec2D(x_info+290,y_info+15),ol::Vec2D(30,20));
    ol::Rgba CouleurPreviewHue(r_pick,v_pick,b_pick,255);
    VisuHue.Draw(CouleurPreviewHue);
    ol::Rgba CouleurPreviewChroma(my_red,my_green,my_blue,255);
    ol::Rect VisuChroma(ol::Vec2D(x_info+325,y_info+15),ol::Vec2D(30,20));
...
{
ol::Canvas::SetClipping(0,ChannelYMenu+hauteur_ChannelMenu,largeur_ecran,hauteur_ecran);
...
int index_posView=0;
int pos_y_vision=0;
ol::Canvas::SetClipping(XChannels,ChannelYMenu+hauteur_ChannelMenu ,XChannels+600 , hauteur_ecran);
...
ol::Rect Registered(ol::Vec2D(xVisuBanger-5, 272),ol::Vec2D(145,18));
...
//retour sauvegarde
ol::Rect RetourConduite(ol::Vec2D(xVisuBanger,330),ol::Vec2D(270,40));
...
{
ol::Rect nameAera( ol::Vec2D( 670, 27), ol::Vec2D( 380,40));
...
ol::Rect UnderText(ol::Vec2D(670,32),ol::Vec2D((numeric_postext*14)+50,30));
...
ol::Rect ProcedureAera( ol::Vec2D( window_proc_x, window_proc_y), ol::Vec2D( 400,70));
...
int DoMouse()
{
    ...
    triangle3d_f(screen, POLYTYPE_GCOL, NULL, &mousev1, &mousev2, &mousev3);
     ...
     ol::Line( ol::Vec2D( mouse_x-1,mouse_y-1), ol::Vec2D( mouse_x+20,mouse_y+30),2.0).Draw( CouleurLigne );
     ol::Line( ol::Vec2D( mouse_x+20,mouse_y+30), ol::Vec2D( mouse_x+30,mouse_y+10),2.0).Draw( CouleurLigne );
     ol::Line( ol::Vec2D( mouse_x+30,mouse_y+10), ol::Vec2D( mouse_x-1,mouse_y-1),2.0).Draw(CouleurLigne);
