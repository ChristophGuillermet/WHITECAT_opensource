...
{
ol::Rect Tracker(ol::Vec2D(videoX+tracking_coordonates[tracking_dock_selected][Tr][0]+15, videoY+tracking_coordonates[tracking_dock_selected][Tr][1]+61 ),
             ol::Vec2D(tracking_coordonates[tracking_dock_selected][Tr][2],tracking_coordonates[tracking_dock_selected][Tr][3]));
...
ol::Rect Background_Editor(ol::Vec2D(videoX+15, videoY+70),ol::Vec2D(video_size_x,40));
...
ol::Rect Background_Thres(ol::Vec2D(videoX+30, videoY+70),ol::Vec2D(265,hauteur_cadre_filter_image));

Background_Thres.SetRoundness(5);
Background_Thres.Draw(CouleurFond.WithAlpha(0.3));

ol::Line(videoX+40, videoY+100,videoX+277,videoY+100,2.0).Draw( CouleurSurvol);
ol::Rect Curseur_Thres(ol::Vec2D(videoX+20+threshold_level, videoY+90),ol::Vec2D(40,25));
Curseur_Thres.SetRoundness(7.5);
Curseur_Thres.Draw(CouleurSurvol);

ol::Line(videoX+40, videoY+160,videoX+277,videoY+160,2.0).Draw( CouleurSurvol);
ol::Rect Curseur_Erode(ol::Vec2D(videoX+20+(erode_level*25), videoY+150),ol::Vec2D(40,25));
Curseur_Erode.SetRoundness(7.5);
Curseur_Erode.Draw(CouleurSurvol);


ol::Rect Curseur_div(ol::Vec2D(videoX+20+(div_facteur), videoY+210),ol::Vec2D(40,25));
Curseur_div.SetRoundness(7.5);
if(ocv_calcul_mode==0)
{
ol::Line(videoX+40, videoY+220,videoX+277,videoY+220,2.0).Draw( CouleurSurvol);
...
ol::Rect OnFlip(ol::Vec2D(videoX+30, videoY+video_size_y+110),ol::Vec2D(60,25));
OnFlip.SetRoundness(7.5);
OnFlip.SetLineWidth(epaisseur_ligne_fader);
OnFlip.Draw(CouleurFond);

ol::Rect OnThreshold(ol::Vec2D(videoX+100, videoY+video_size_y+110),ol::Vec2D(60,25));
OnThreshold.SetRoundness(7.5);
OnThreshold.SetLineWidth(epaisseur_ligne_fader);
OnThreshold.Draw(CouleurFond);

ol::Rect ModeErode(ol::Vec2D(videoX+170, videoY+video_size_y+110),ol::Vec2D(60,25));
ModeErode.SetRoundness(7.5);
ModeErode.SetLineWidth(epaisseur_ligne_fader);
ModeErode.Draw(CouleurFond);

ol::Rect OnOffBlur(ol::Vec2D(videoX+240, videoY+video_size_y+110),ol::Vec2D(60,25));
...
ol::Point::StartFastDrawing();
                   ...
                   ol::Rgba CouleurPixel(recup_val_pix_video,recup_val_pix_video,recup_val_pix_video);
                   ol::Point( ol::Vec2D(videoX+15+pix_x,videoY+61+pix_y)).DrawFast(CouleurPixel);
                   }
                   }
...
//background window
ol::Rect VideoBackground(ol::Vec2D( WindowVideoX, WindowVideoY ), ol::Vec2D( frame_video_x+35,frame_video_y+460));
...
//FPS CALIBRATION///////////////////////////////////////////////////////////////
ol::Rect FPSVideo(ol::Vec2D(WindowVideoX+120+(fps_video_rate*3), WindowVideoY+10), ol::Vec2D( 60,25));
FPSVideo.SetRoundness(10);
FPSVideo.SetLineWidth(epaisseur_ligne_fader);
ol::Line(WindowVideoX+150, WindowVideoY+45,WindowVideoX+330, WindowVideoY+45,2.0).Draw(CouleurLigne);
ol::Line(WindowVideoX+150+(fps_video_rate*3), WindowVideoY+35,WindowVideoX+150+(fps_video_rate*3), WindowVideoY+45,2.0).Draw(CouleurLigne);
...
//cadre image
ol::Rect PourtourImage(ol::Vec2D(videoX+15, videoY+60 ), ol::Vec2D( frame_video_x,frame_video_y));
...
{
//ligne 1
ol::Rect Tracker(ol::Vec2D(WindowVideoX+20+(ct*60), WindowVideoY+frame_video_y+200), ol::Vec2D( 50,30));
...
//ligne2
ol::Rect TrackerTwo(ol::Vec2D(WindowVideoX+20+(ct*60), WindowVideoY+frame_video_y+250), ol::Vec2D( 50,30));
...
///////////////OPTIONS///////////////////////////////////////////////////////////////
//mode
ol::Rect OcvMode(ol::Vec2D(WindowVideoX+30, WindowVideoY+frame_video_y+150),ol::Vec2D(60,30));
...
ol::Rect TakeSnap(ol::Vec2D(WindowVideoX+100, WindowVideoY+frame_video_y+150 ), ol::Vec2D(60,30));
...
ol::Rect FilteringOn(ol::Vec2D(WindowVideoX+180, WindowVideoY+frame_video_y+150), ol::Vec2D( 100,30));
...
/////////
ol::Rect ClearMode(ol::Vec2D(WindowVideoX+300, WindowVideoY+frame_video_y+150),ol::Vec2D(60,30));
...
{
ol::Rect DockingTracker(ol::Vec2D(WindowVideoX+20+(ct*70), WindowVideoY+frame_video_y+370), ol::Vec2D( 60,30));
...
ol::Rect EditTrack(ol::Vec2D(WindowVideoX+30, WindowVideoY+frame_video_y+310 ), ol::Vec2D(60,30));
EditTrack.SetRoundness(7.5);
EditTrack.SetLineWidth(epaisseur_ligne_fader);
ol::Rect MoveTrack(ol::Vec2D(WindowVideoX+110, WindowVideoY+frame_video_y+310 ), ol::Vec2D(60,30));
...
//INDEX DECAY TRACKER
ol::Rect DecaySlider( ol::Vec2D(videoX+190+index_decay_tracker,videoY+frame_video_y+310),ol::Vec2D(20,30));
DecaySlider.SetRoundness(5);

ol::Rect BackSliderDecay( ol::Vec2D(videoX+190,videoY+frame_video_y+310),ol::Vec2D(147,30));
BackSliderDecay.SetRoundness(7.5);
BackSliderDecay.DrawOutline(CouleurLigne.WithAlpha(0.5));

ol::Line(ol::Vec2D(videoX+200,videoY+frame_video_y+325),ol::Vec2D(videoX+327,videoY+frame_video_y+325)).Draw(CouleurLigne);
...
ol::Circle DMidiOut(videoX+365,videoY+frame_video_y+350, 10);//box du fader
DMidiOut.SetLineWidth(epaisseur_ligne_fader);
//liaison visuelle au fader du cercle midi out
ol::Line (ol::Vec2D(videoX+337,videoY+frame_video_y+330),ol::Vec2D( videoX+352,videoY+frame_video_y+330)).Draw(CouleurLigne);
ol::Line (ol::Vec2D( videoX+352,videoY+frame_video_y+330),ol::Vec2D( videoX+367,videoY+frame_video_y+350)).Draw(CouleurLigne);
...
ol::Rect AffectVideoTo( ol::Vec2D(videoX+(frame_video_y/2)-25,videoY+frame_video_y+417),ol::Vec2D(130,30));
