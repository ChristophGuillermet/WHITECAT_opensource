...
for (int df=0;df<5;df++)
{
ol::Rect FunctionChThr(ol::Vec2D(fx, fy + (df*fhaut)+ (space*df)), ol::Vec2D( flarg,fhaut));
...
{
ol::Rect MyMainMenu(ol::Vec2D(xmenu,ymenu),ol::Vec2D(size_x_mainmenu,size_y_mainmenu));
...
// TEXT autoclose
ol::Rect Autoclose( ol::Vec2D( xmenu+330,ymenu+8), ol::Vec2D( 30,15));
