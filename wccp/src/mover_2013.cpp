...
int Print_Points(int move_selected)
{
    ...
    {
    ol::Circle CercleShowPoint( ol::Vec2D(dock_move_xy[move_selected][dms][0]+xmover_window+20, dock_move_xy[move_selected][dms][1]+ymover_window+20), 4 );
    ol::Rect ShowInPoint(ol::Vec2D(dock_move_xy[move_selected][dms][0]+xmover_window+12, dock_move_xy[move_selected][dms][1]+ymover_window+12), ol::Vec2D( 16,16));

    if(dms==Moves_Inpoint[move_selected]){ShowInPoint.Draw(CouleurLock);}
    CercleShowPoint.Draw(ol::Rgba::WHITE);
...
{
ol::Rect ParamN(ol::Vec2D( xt+60,yt+5+(dv*15)),ol::Vec2D(30,12));
ol::Rect LevelFrame(ol::Vec2D(xt+180,yt+5+(dv*15)),ol::Vec2D(255,10));
...
ol::Rect Level(ol::Vec2D(xt+180,yt+5+(dv*15)),ol::Vec2D(v_tmp,10));
...
if(dv>=2)
{
ol::Rect AllowCopyParams(ol::Vec2D(xt+445,yt+5+(dv*15)),ol::Vec2D(10,10));
...
ol::Rect ShiftIris(ol::Vec2D(xt+5,yt+130),ol::Vec2D(100,20));
ShiftIris.SetRoundness(4);
ShiftIris.Draw(CouleurFader.WithAlpha(index_copy_params));
ShiftIris.DrawOutline(ol::Rgba::WHITE);
...
//border
ol::Rect BorderTrckWindow(ol::Vec2D(track_aera_x-10,track_aera_y-10),ol::Vec2D(275,275));
BorderTrckWindow.SetRoundness(15);
BorderTrckWindow.SetLineWidth(demi_epaisseur_ligne_fader);
BorderTrckWindow.DrawOutline(ol::Rgba::WHITE);

//tracking zone
ol::Rect TrckWindow(ol::Vec2D(track_aera_x,track_aera_y),ol::Vec2D(255,255));
...
//affichage du tracker
 ...
 ol::Circle monTracker( ol::Vec2D(mover_params[0][0]+track_aera_x, mover_params[1][0]+track_aera_y), 10 );
 monTracker.SetLineWidth(epaisseur_ligne_fader);

//affichage du tracker fine ( 16 bits)
 ol::Circle monTrackerfine( ol::Vec2D(mover_params[0][1]+track_aera_x, mover_params[1][1]+track_aera_y), 10 );
...
{TrckWindow.Draw(CouleurBleuProcedure.WithAlpha(0.5));
 monTrackerfine.DrawOutline(ol::Rgba::YELLOW );}


//mire
ol::Line(ol::Vec2D(track_aera_x+127,track_aera_y), ol::Vec2D( track_aera_x+127,track_aera_y+255)).Draw(CouleurLigne);
ol::Line(ol::Vec2D(track_aera_x,track_aera_y+127), ol::Vec2D( track_aera_x+255,track_aera_y+127)).Draw(CouleurLigne);
ol::Circle CercleMire( ol::Vec2D(track_aera_x+127, track_aera_y+127), 127 );
CercleMire.DrawOutline(ol::Rgba::WHITE );


//affichage ZERO POINT
 ol::Circle CercleZeroPoint( ol::Vec2D(position_defaut[0]+track_aera_x+127,position_defaut[1]+track_aera_y+127), 6 );
 CercleZeroPoint.Draw(ol::Rgba::BLACK.WithAlpha(0.3));
...
if(tracker_16b_edit==0)
{
ol::Circle CercleShowIris( ol::Vec2D(track_aera_x+127,track_aera_y+127), (int)(mover_params[2][0]/2) );
CercleShowIris.Draw(ol::Rgba::WHITE.WithAlpha(0.2));
ol::Circle CercleShowFocus( ol::Vec2D(track_aera_x+127,track_aera_y+127), (int)(mover_params[3][0]/2) );
CercleShowFocus.Draw(ol::Rgba::BLUE.WithAlpha(0.2));
ol::Circle CercleShowZoom( ol::Vec2D(track_aera_x+127,track_aera_y+127), (int)(mover_params[4][0]/2) );
CercleShowZoom.SetLineWidth(demi_epaisseur_ligne_fader);
CercleShowZoom.DrawOutline(ol::Rgba::BLACK);
}
else if(tracker_16b_edit==1)
{
ol::Circle CercleShowIris( ol::Vec2D(track_aera_x+127,track_aera_y+127), (int)(mover_params[2][1]/2) );
CercleShowIris.Draw(ol::Rgba::WHITE.WithAlpha(0.2));
ol::Circle CercleShowFocus( ol::Vec2D(track_aera_x+127,track_aera_y+127), (int)(mover_params[3][1]/2) );
CercleShowFocus.Draw(ol::Rgba::BLUE.WithAlpha(0.2));
ol::Circle CercleShowZoom( ol::Vec2D(track_aera_x+127,track_aera_y+127), (int)(mover_params[4][1]/2) );
CercleShowZoom.SetLineWidth(demi_epaisseur_ligne_fader);
CercleShowZoom.DrawOutline(ol::Rgba::BLACK);
}
...
{
monTrackerfine.Draw(ol::Rgba::YELLOW );
if(mouse_button==1)
{
index_mouse_is_tracking=1;
...
//SPEED SLIDER
//FADER
ol::Rect SliderFaderSpeed( ol::Vec2D( x_track+420,y_track+350 ), ol::Vec2D( 50,255));//box du fader
SliderFaderSpeed.SetRoundness(15);
SliderFaderSpeed.SetLineWidth(epaisseur_ligne_fader);
ol::Rect LevelFaderSpeed( ol::Vec2D( x_track+420,y_track+350+255-Tracker_speedlevel ), ol::Vec2D( 50,Tracker_speedlevel));//box du fader
...
ol::Rgba Cycling(1.0,0.0+color_cycling,0.0);
LevelFaderSpeed.Draw(Cycling);
}
SliderFaderSpeed.DrawOutline(ol::Rgba::WHITE);
...
{
//ligne1
ol::Rect StepActionL1(ol::Vec2D(x_track+20+(lopt*40),track_aera_y+275),ol::Vec2D(30,20));

//ligne2
ol::Rect StepActionL2(ol::Vec2D(x_track+20+(lopt*40),track_aera_y+305),ol::Vec2D(30,20));

switch(lopt)
{
case 0:
sprintf(string_titre_stepsL1,"Add");sprintf(string_titre_stepsL2," <<<");
StepActionL1.DrawOutline(ol::Rgba::WHITE);
StepActionL2.DrawOutline(ol::Rgba::WHITE);
break;
case 1:
...
//PLAY BACK
ol::Rect PlayBackMove( ol::Vec2D( x_track+20,y_track+360 ), ol::Vec2D( 60,25));//box du fader
...
PlayBackMove.DrawOutline(ol::Rgba::WHITE);
petitchiffre.Print("GO BACK",x_track+25, y_track+375);

//PLAY GO
ol::Rect PlayGoMove( ol::Vec2D( x_track+100,y_track+360 ), ol::Vec2D( 60,25));//box du fader
...
PlayGoMove.DrawOutline(ol::Rgba::WHITE);
petitchiffre.Print("GO",x_track+120, y_track+375);


//CYCLE OR NOT CYCLE
ol::Rect CycleMove( ol::Vec2D( x_track+180,y_track+360 ), ol::Vec2D( 60,25));//box du fader
...
CycleMove.DrawOutline(ol::Rgba::WHITE);
...
/////////////SPLINE//////////////////////////////////////////////////////////////////////////////
ol::Rect SplineOn( ol::Vec2D( x_track+260,y_track+360 ), ol::Vec2D( 60,25));//box du fader
...
SplineOn.DrawOutline(ol::Rgba::WHITE);
...
//spline ratio
ol::Rect SliderFaderSpline( ol::Vec2D( x_track+230,y_track+400 ), ol::Vec2D( 127,20));//box du fader
SliderFaderSpline.SetRoundness(3);
ol::Rect LevelFaderSpline( ol::Vec2D( x_track+230,y_track+400) , ol::Vec2D( Tracker_splinelevel,20));//box du fader
...
SliderFaderSpline.DrawOutline(ol::Rgba::WHITE);
ol::Line(ol::Vec2D(x_track+230+63,y_track+400), ol::Vec2D(x_track+230+63,y_track+400+20)).Draw(CouleurLigne);
ol::Line(ol::Vec2D(x_track+230+65,y_track+400), ol::Vec2D(x_track+230+65,y_track+400+20)).Draw(CouleurLigne);

///////////////////////TIME SHIFT REDISTRIBUTION //////////////////////////////////////////////
ol::Rect Timeshift(ol::Vec2D(x_track+300,y_track+490),ol::Vec2D(70,20));
...
Timeshift.DrawOutline(ol::Rgba::WHITE);
...
{
ol::Rect DockMove( ol::Vec2D( x_track+300+(dkmov*40),y_track+(dkline*30) ), ol::Vec2D( 30,25));//box du fader
...
DockMove.DrawOutline(ol::Rgba::WHITE);
}
}
...
//affichage au dessus du tracker
  monTracker.DrawOutline(ol::Rgba::WHITE );
...
///////////////////////////AFFCECT TO DOCK///////////////////////////////////////
ol::Rect AffectToDock( ol::Vec2D( x_track+310,y_track+250 ), ol::Vec2D( 150,25));//box du fader
...
//TOGGLE 8 / 16 BITS
ol::Rect BitMode(ol::Vec2D(track_aera_x+20,track_aera_y+380),ol::Vec2D(60,25));
...
BitMode.DrawOutline(ol::Rgba::WHITE);
...
//EDIT FINE ON OFF
ol::Rect EditFine(ol::Vec2D(track_aera_x+20,track_aera_y+415),ol::Vec2D(60,25));
...
EditFine.DrawOutline(ol::Rgba::WHITE);

// LOCK X ou Y
ol::Rect LockX(ol::Vec2D(track_aera_x+20,track_aera_y+460),ol::Vec2D(60,25));
LockX.SetRoundness(7.5);
LockX.SetLineWidth(epaisseur_ligne_fader);
ol::Rect LockY(ol::Vec2D(track_aera_x+100,track_aera_y+460),ol::Vec2D(60,25));
...
LockX.DrawOutline(ol::Rgba::WHITE);
LockY.DrawOutline(ol::Rgba::WHITE);

///////////// A AFFECTER AU DEVICE ///////////////////

// INVERSE X Y DATA
ol::Rect InvX(ol::Vec2D(track_aera_x+20,track_aera_y+500),ol::Vec2D(60,25));
InvX.SetRoundness(7.5);
InvX.SetLineWidth(epaisseur_ligne_fader);
ol::Rect InvY(ol::Vec2D(track_aera_x+100,track_aera_y+500),ol::Vec2D(60,25));
...
InvX.DrawOutline(ol::Rgba::WHITE);
InvY.DrawOutline(ol::Rgba::WHITE);


/// OFFSET position_defaut[2]
ol::Rect ZeroPoint(ol::Vec2D(x_track+40,y_track+566),ol::Vec2D(140,25));
...
ZeroPoint.DrawOutline(ol::Rgba::WHITE);
