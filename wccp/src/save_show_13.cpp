...
{
ol::Rect Report_Save_Load(ol::Vec2D(report_SL_X, report_SL_Y), ol::Vec2D( 350,160));
...
ol::Rect MinusPosMyRep(ol::Vec2D(report_SL_X+290,report_SL_Y+76),ol::Vec2D(50,20));
MinusPosMyRep.SetRoundness(7.5);
MinusPosMyRep.SetLineWidth(epaisseur_ligne_fader);
ol::Rect PlusPosMyRep(ol::Vec2D(report_SL_X+290,report_SL_Y+106),ol::Vec2D(50,20));
PlusPosMyRep.SetRoundness(7.5);
PlusPosMyRep.SetLineWidth(epaisseur_ligne_fader);

for(int co=0;co<10;co++)
{
ol::Rect AlarmFile(ol::Vec2D((report_SL_X+5),(report_SL_Y+52+(10*co))),ol::Vec2D(300,12));
AlarmFile.SetRoundness(5);
if(b_report_error[co+position_view_line]==1)
{
 AlarmFile.Draw(ol::Rgba::RED.WithAlpha(alpha_blinker));
}
...
fprintf(fpp,"#arguments 2ndol::Line :GPL NUM ( 1 to 4 ) / Offset ( 1 to 513 ) / position point / col / row /\n");
...
fprintf(fpp,"#arguments: Grid Scale / Plot Window size X / Plot Window size X / Plot Viewport X / Plot Viewport Y / ol::Line Color / Background Color / Number of shapes present / Show shapes id / Grid Type / Alpha Grid / Alpha Shapes / Plan ratio / SoloMode xy relativs /\n");
...
ol::Rgba TmpPlotColor (Color_plotline,Color_plotline,Color_plotline);
CouleurPlotLine=TmpPlotColor;
ol::Rgba TmpPlotColor2 (Color_plotfill,Color_plotfill,Color_plotfill);
...
fclose(fpA);
 ...
 {
  ol::Rgba TmpColor (couleurs_user[tmp][0],couleurs_user[tmp][1],couleurs_user[tmp][2]);
