...
{
int rayon=15;
 ol::Circle ChannelProp(ol::Vec2D(plotx,ploty),rayon);
...
//up
ol::Line(ol::Vec2D(plotx+12,ploty+50),ol::Vec2D(plotx+28,ploty+50)).Draw(CouleurNoir);
ol::Line(ol::Vec2D(plotx+12,ploty+50),ol::Vec2D(plotx+20,ploty+36)).Draw(CouleurNoir);
ol::Line(ol::Vec2D(plotx+28,ploty+50),ol::Vec2D(plotx+20,ploty+36)).Draw(CouleurNoir);
//down
ol::Line(ol::Vec2D(plotx+12,ploty+69),ol::Vec2D(plotx+28,ploty+69)).Draw(CouleurNoir);
ol::Line(ol::Vec2D(plotx+12,ploty+69),ol::Vec2D(plotx+20,ploty+83)).Draw(CouleurNoir);
ol::Line(ol::Vec2D(plotx+28,ploty+69),ol::Vec2D(plotx+20,ploty+83)).Draw(CouleurNoir);
...
ol::Rect symbol_sizer_frame(ol::Vec2D(plotx+50,ploty+55),ol::Vec2D(50,10));
ol::Rect symbol_sizer_level(ol::Vec2D(plotx+50,ploty+55),ol::Vec2D(50.0*size_symbol[symbol_selected_type],10));
switch(index_edit_symbol_plot)
{
case 0:
...
for(int i=0;i<4;i++)
{
PlotOptionView.MoveTo(ol::Vec2D(plotx+5,ploty+220+(20*i)));
if(adjust_xy_type==i)
{
...
ol::Rect AeraDrawAdjust(ol::Vec2D(plotx+110,ploty+220),ol::Vec2D(70,70));
AeraDrawAdjust.DrawOutline(CouleurPlotLine.WithAlpha(0.3));
ol::Rect AeraTemoin(ol::Vec2D(plotx+140+rlativ_xm,ploty+250+rlativ_ym),ol::Vec2D(10,10));
...
//repatch directly
PlotOptionsEdit_Long.MoveTo(ol::Vec2D(plotx+5,ploty+300));
...
{
ol::Rect PlotSymbEd(ol::Vec2D(plotx,ploty),ol::Vec2D(200,380));
...
AnglePos.MoveTo(ol::Vec2D(plotx+140+15,ploty+10+15));
AnglePos.Draw(CouleurPlotFill);
AnglePos.DrawOutline(CouleurPlotFill);


ol::Rect MySymbAction(ol::Vec2D(plotx, ploty+70),ol::Vec2D(60,18));
...
for(int l=0;l<2;l++)
{
MySymbAction.MoveTo(ol::Vec2D(plotx+5+(c*65),ploty+70+(l*20)));
...
AnglePos.MoveTo(ol::Vec2D(plotx+140+15,ploty+125+15));
AnglePos.Draw(CouleurPlotFill);
AnglePos.DrawOutline(CouleurPlotFill);
...
GridBGSelector.MoveTo(ol::Vec2D(plotx+5+50,ploty+480));
GridBGSelector.Draw(CouleurPlotLine.WithAlpha(0.3));
...
{
Line
(ol::Vec2D(plotx+(i*plot_quadrillage_size),ploty),ol::Vec2D(plotx+(i*plot_quadrillage_size),ploty+(plot_window_y_size-position_plan_y))).Draw(CouleurPlotLine);
}
for(int i=0;i<=plot_nbre_lin;i++)
{
Line
(ol::Vec2D(plotx,ploty+(i*plot_quadrillage_size)),ol::Vec2D(plotx+(plot_window_x_size-position_plan_x),ploty+(i*plot_quadrillage_size))).Draw(CouleurPlotLine);
}
}
return(0);
}
