...
ol::Rect StepB(ol::Vec2D(xb+22,yb-5),ol::Vec2D(35,30));
StepB.SetRoundness(4);
StepB.DrawOutline(CouleurLigne.WithAlpha(0.4));
ol::Rect Step127(ol::Vec2D(xb+60,yb-6),ol::Vec2D(20,5));
...
ol::Rect StepMinus(ol::Vec2D(xb,yb),ol::Vec2D(20,20));
...
ol::Rect StepPlus(ol::Vec2D(xb+60,yb),ol::Vec2D(20,20));
...
//affichage progression transfert
 ol::Rect BackSt(ol::Vec2D(xb,yb-5),ol::Vec2D(63,10));
 ol::Rect BackPreset(ol::Vec2D(xb,yb+10),ol::Vec2D(63,10));
 BackSt.DrawOutline(CouleurLigne.WithAlpha(0.2));
 BackPreset.DrawOutline(CouleurLigne.WithAlpha(0.2));
 ol::Rect ActStage(ol::Vec2D(xb,yb-5),ol::Vec2D(grid_niveauX1[num_grid_player]/4,10));
 ActStage.Draw(CouleurNiveau.WithAlpha(0.7));
 ActStage.Draw(CouleurLigne.WithAlpha(0.5));
 ol::Rect ActPreset(ol::Vec2D(xb,yb+10),ol::Vec2D(grid_niveauX2[num_grid_player]/4,10));
...
BackPreset.Draw(CouleurBlind.WithAlpha(alpha_blinker));
 ...
 ol::Rect AcelGrid(ol::Vec2D(xb,yb+30),ol::Vec2D(127,10));
 AcelGrid.DrawOutline(CouleurLigne.WithAlpha(0.5));
 ol::Rect AccelB(ol::Vec2D(xb+grid_crossfade_speed[num_grid_player]-5,yb+30),ol::Vec2D(10,10));
 AccelB.SetRoundness(2.0);
 AccelB.Draw(CouleurFader);
 ol::Line(ol::Vec2D(xb+64,yb+25),ol::Vec2D(xb+64,yb+45)).DrawOutline(CouleurLigne);
...
//next step
 ...
 ol::Rect SlaveB(ol::Vec2D(xb+150,yb+25),ol::Vec2D(40,15));
...
SlaveB.DrawOutline(CouleurBlind);
 ...
 ol::Rect CadreStep(ol::Vec2D(xb+15,yb+50),ol::Vec2D(180,100));
 ...
 {
 ol::Rect OnOffSp(ol::Vec2D(xb,yb+70+(sp*20)),ol::Vec2D(10,10));
...
{
 ol::Rect AffectToDocB(ol::Vec2D(xb+60,yb),ol::Vec2D(50,30));
 ...
 ol::Rect ToStepMB(ol::Vec2D(xb,yb+45),ol::Vec2D(10,10));
 ...
 ol::Rect StepToB(ol::Vec2D(xb+60,yb+45),ol::Vec2D(40,15));
 ...
 ///////////COPY TO ////////////////////////////////////////////////////////////
 ol::Line(ol::Vec2D(xb,yb+68),ol::Vec2D(xb+110,yb+68)).Draw(CouleurLigne.WithAlpha(0.5));



 ol::Rect CopyB(ol::Vec2D(xb,yb+75),ol::Vec2D(50,15));
 CopyB.SetRoundness(4.0);
 CopyB.Draw(CouleurSurvol.WithAlpha(0.5));

 petitpetitchiffre.Print("CopyTo",xb+5,yb+85);

 ol::Rect InsB(ol::Vec2D(xb+55,yb+75),ol::Vec2D(50,15));
 InsB.SetRoundness(4.0);
 InsB.Draw(CouleurSurvol.WithAlpha(0.5));
 petitpetitchiffre.Print("Insert",xb+60,yb+85);


 ol::Rect GridCopyB(ol::Vec2D(xb,yb+95),ol::Vec2D(30,15));
 GridCopyB.SetRoundness(0.5);
 GridCopyB.DrawOutline(CouleurLigne.WithAlpha(0.2));

 ol::Rect StepCopyB(ol::Vec2D(xb+45,yb+95),ol::Vec2D(40,15));
...
{
ol::Rect BoxLink(ol::Vec2D(xb, yb+115),ol::Vec2D(105,15));
...
ol::Rect GetDraw(ol::Vec2D(xb, yb+135),ol::Vec2D(105,15));
...
{
ol::Line(ol::Vec2D(xb+(co*15),yb),ol::Vec2D(xb+(co*15),yb+(grider_nb_row*15))).Draw(CouleurLigne.WithAlpha(0.5));
}

for(int lo=0;lo<=grider_nb_row;lo++)
{
ol::Line(ol::Vec2D(xb,yb+(lo*15)),ol::Vec2D(xb+(grider_nb_col*15),yb+(lo*15))).Draw(CouleurLigne.WithAlpha(0.5));
}
...
ol::Rect QuadCh(ol::Vec2D(tmpx,tmpy),ol::Vec2D(15,15));
...
{
ol::Rect QuadPrev(ol::Vec2D(tmpx,tmpy),ol::Vec2D(5,5));
QuadPrev.Draw(CouleurSurvol);
}
if(grid_levels[grid_in_preset[num_grid_player][0]][grid_in_preset[num_grid_player][1]][tempfu]>0)
{
ol::Rect QuadNext(ol::Vec2D(tmpx+10,tmpy),ol::Vec2D(5,5));
QuadNext.Draw(CouleurLigne);
}
}
}

ol::Rect ShowOffset(ol::Vec2D(0,0),ol::Vec2D(15,15));
for(int pr=0;pr<6;pr++)
{
 if( draw_brush_type[pr]==1 && draw_get_gpl[pr]==num_grid_player+1)//si GPL tracké depuis DRAW
 {
 ShowOffset.MoveTo(ol::Vec2D(xb+ (((draw_offset_gpl[pr]-1)%grider_nb_col)*15) , yb+ (((draw_offset_gpl[pr]-1)/grider_nb_col)*15)   ));
...
ol::Rect Backchiffres(ol::Vec2D(mouse_x-5, mouse_y-25),ol::Vec2D(40,25));
...
{
ol::Rect GriderNum(ol::Vec2D(xb+10,yb),ol::Vec2D(50,20));
...
ol::Rect GiveNameOfGrider(ol::Vec2D(xb+70,yb), ol::Vec2D( 170,20));
...
ol::Circle GriderPlus(ol::Vec2D(xb+295,yb+10),12);
ol::Circle GriderMinus(ol::Vec2D(xb+265,yb+10),12);
...
{
ol::Line(ol::Vec2D(xb+10,yb-10),ol::Vec2D(xb+largeurGrider, yb-10)).Draw(CouleurLigne);
...
{
//appel de grille
ol::Line(ol::Vec2D(xb,yb),ol::Vec2D(xb+largeurGrider,yb)).Draw(CouleurLigne);
neuromoyen.Print("Grid",xb,yb+23);
ol::Rect GriderNum(ol::Vec2D(xb+40,yb+10),ol::Vec2D(50,20));
GriderNum.SetRoundness(5);
GriderNum.DrawOutline(CouleurLigne.WithAlpha(0.5));
...
ol::Rect GriderSt(ol::Vec2D(xb+150,yb+10),ol::Vec2D(50,20));
...
for (int co=0;co<=grider_nb_col;co++)
{
ol::Line(ol::Vec2D(20+xb+(co*5)+(pos*grider_nb_col*5)+(20*pos),yb+40),ol::Vec2D(20+xb+(co*5)+(pos*grider_nb_col*5)+(20*pos),yb+40+(grider_nb_row*5))).Draw(CouleurLigne.WithAlpha(0.5));
}

for(int lo=0;lo<=grider_nb_row;lo++)
{
ol::Line(ol::Vec2D(20+xb+(pos*grider_nb_col*5)+(20*pos),yb+40+(lo*5)),ol::Vec2D(20+xb+(pos*grider_nb_col*5)+(grider_nb_col*5)+(20*pos),yb+40+(lo*5))).Draw(CouleurLigne.WithAlpha(0.5));
}
...
{
ol::Rect QuadChF(ol::Vec2D(tmpx,tmpy),ol::Vec2D(5,5));
...
{
ol::Rect QuadCh(ol::Vec2D(tmpx,tmpy),ol::Vec2D(5,5));
QuadCh.Draw(CouleurGreen);
}
}
}
neuromoyen.Print(ol::ToString(tempstep+1),tmpblx,tmpbly);
ol::Rect AutoStopB(ol::Vec2D(tmpblx+40,tmpbly-10),ol::Vec2D(30,10));
...
{
ol::Rect GriderBack(ol::Vec2D(xb,yb),ol::Vec2D(largeurGrider,hauteurGrider));
...
ol::Rect BackGriderChan(ol::Vec2D(xb+245,yb+15),ol::Vec2D(40,20));
...
ol::Rect BackGriderCol(ol::Vec2D(xb+315,yb+15),ol::Vec2D(30,20));
...
ol::Rect BackGriderRow(ol::Vec2D(xb+385,yb+15),ol::Vec2D(30,20));
...
//////////////EDIT MODE///////////////////////////////
ol::Rect GriderEditEnable( ol::Vec2D((xb+430),(yb+15)),ol::Vec2D(50,20));
...
{
ol::Rect ShowGridB(ol::Vec2D(xb+540+(o*15),yb+20),ol::Vec2D(10,10));
...
ol::Rect AllowGridViewerB(ol::Vec2D(xb+495,yb+20),ol::Vec2D(40,10));
