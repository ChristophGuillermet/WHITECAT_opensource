...
ol::Rect fond_my_window(ol::Vec2D(mx,my), ol::Vec2D( largeur_my_window,hauteur_my_window));
...
ol::Rect FaderFrame(ol::Vec2D(mx,my),ol::Vec2D(20,127));
FaderFrame.SetRoundness(3);

for(int i=0;i<6;i++)
{
petitchiffre.Print(ol::ToString(my_window_fader_value[i] ),mx+12+(i*35),my+58);
FaderFrame.MoveTo(ol::Vec2D(mx+10+(i*35),my+70));
ol::Rect FaderLevel(ol::Vec2D(mx+10+(i*35),my+70+127-my_window_fader_value[i] ), ol::Vec2D(20,my_window_fader_value[i] ));
