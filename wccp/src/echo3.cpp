...
{
//Aera channels
ol::Rect Aera(ol::Vec2D(xe,ye),ol::Vec2D(500,127));
Aera.SetRoundness(4);
Aera.Draw(Discrete8);

ol::Rect PreviousChan(ol::Vec2D(xe,ye),ol::Vec2D(25,127));
PreviousChan.SetRoundness(4);
ol::Line(ol::Vec2D(xe+5,ye+64),ol::Vec2D(xe+15,ye+54)).Draw(CouleurLigne);
ol::Line(ol::Vec2D(xe+15,ye+54),ol::Vec2D(xe+15,ye+74)).Draw(CouleurLigne);
ol::Line(ol::Vec2D(xe+15,ye+74),ol::Vec2D(xe+5,ye+64)).Draw(CouleurLigne);


ol::Rect NextChan(ol::Vec2D(xe+475,ye),ol::Vec2D(25,127));
NextChan.SetRoundness(4);
ol::Line(ol::Vec2D(xe+490,ye+64),ol::Vec2D(xe+480,ye+54)).Draw(CouleurLigne);
ol::Line(ol::Vec2D(xe+480,ye+54),ol::Vec2D(xe+480,ye+74)).Draw(CouleurLigne);
ol::Line(ol::Vec2D(xe+480,ye+74),ol::Vec2D(xe+490,ye+64)).Draw(CouleurLigne);


ol::Rect ChanEcho(ol::Vec2D(xe+475,ye),ol::Vec2D(30,6));
...
for (int sh=0;sh<15;sh++)
{
ol::Line(ol::Vec2D(xe+25+(sh*30),ye),ol::Vec2D(xe+25+(sh*30),ye+127)).Draw(CouleurLigne.WithAlpha(0.7));

echochanis=echo_grid_channel_position[echo_selected]+sh;


//ground
ol::Rect MassE(ol::Vec2D(xe+25+(sh*30),ye+(127*(1.0-echo_levels[echo_selected][1][echochanis]))),ol::Vec2D(30,echo_levels[echo_selected][1][echochanis]*127));
MassE.Draw(Discrete2);

//enregistrement level de départ
ChanEcho.MoveTo(ol::Vec2D(xe+25+(sh*30),ye+127-(snap_echo_to_recall[echo_selected][echochanis]*127)));
ChanEcho.Draw(CouleurFond.WithAlpha(0.3));

//niveau
ChanEcho.MoveTo(ol::Vec2D(xe+25+(sh*30),ye+127-(echo_levels[echo_selected][0][echochanis]*127)));
...
ol::Line(ol::Vec2D(xe+25+450,ye),ol::Vec2D(xe+475,ye+127)).Draw(CouleurLigne.WithAlpha(0.7));
...
{
ol::Rect EchoBckgd(ol::Vec2D(xe,  ye), ol::Vec2D( echo_window_x_size,echo_window_y_size));
...
//EDIT
ol::Rect GriderEditEnable( ol::Vec2D((xe+610),(ye+10)),ol::Vec2D(50,20));
...
ol::Rect EchoPreset(ol::Vec2D(xe+10,ye+10),ol::Vec2D(30,15));
...
{
index_ec=cel+(le*8);
EchoPreset.MoveTo(ol::Vec2D(xe+180+(cel*40),ye+10+(le*25)));
...
EchoPreset.DrawOutline(CouleurBlind);
 ...
 ol::Rect SetChannpos(ol::Vec2D(xe+615,ye+65),ol::Vec2D(40,20));
...
//ECHO ACTION PANNEL
ol::Rect Action(ol::Vec2D(xe+10,ye+40),ol::Vec2D(115,20));
Action.SetRoundness(4);
for (int n=0;n<5;n++)
{
Action.MoveTo(ol::Vec2D(xe+10,ye+50+(n*30)));
...
ol::Rect FBox(ol::Vec2D(xe+70,ye+200),ol::Vec2D(45,35));
FBox.SetRoundness(5);
...
////AFFECT TO DOCK
ol::Rect Affecttodock(ol::Vec2D(xe+10,ye+245), ol::Vec2D(110,20));
...
//ECHO ACTION PANNEL
ol::Rect ActionTyp(ol::Vec2D(xe+500,ye+10),ol::Vec2D(70,20));
...
ol::Rect Frame(ol::Vec2D(xe+185,ye+255),ol::Vec2D(127,10));
float tmp_lev=0;


ol::Circle BMidiOut(xe+316,ye+255, 5);
...
ol::Rect LevelF(ol::Vec2D(xe+155+(cl*180),ye+255),ol::Vec2D(tmp_lev*127,10));
LevelF.Draw(Discrete8);
Frame.MoveTo(ol::Vec2D(xe+155+(cl*180),ye+255));
Frame.DrawOutline(CouleurLigne.WithAlpha(0.5));
petitchiffre.Print(tcmp,xe+155+(cl*180),ye+248);
BMidiOut.MoveTo(ol::Vec2D(xe+295+(cl*180),ye+265));
