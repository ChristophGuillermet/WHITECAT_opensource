...
//background window
ol::Rect TrichroBackground(ol::Vec2D(xchroma-158, ychroma-206 ), ol::Vec2D( 315,550));
...
for (hcl=0; hcl<360; hcl+=0.1)
{
	 ...
	 ol::Line(ol::Vec2D(xchroma,ychroma),ol::Vec2D(xchroma+xcl,ychroma+ycl)).Draw(ol::Rgba(rcl,gcl,bcl));
}
...
if(Midi_Faders_Affectation_Type!=0 && window_focus_id==W_TRICHROMY)//config midi
  ...
  {
   ol::Circle Color( ol::Vec2D(xchroma-1,ychroma), 140 );
...
//affichage du pointeur de hue
 ol::Circle monCurseurHue( ol::Vec2D(position_curseur_hue_x, position_curseur_hue_y), 12 );
 monCurseurHue.SetLineWidth(epaisseur_ligne_fader);
 monCurseurHue.DrawOutline( CouleurLigne );


ol::Circle monCurseurTriangle( ol::Vec2D(xchroma+picker_x,ychroma+picker_y), 6 );
...
ol::Rect ChooseTriQuadri( ol::Vec2D(xchroma-60,(ychroma-190)),ol::Vec2D(130,30));
...
ol::Rect AffectTriTo( ol::Vec2D(xchroma+20,(ychroma+300)),ol::Vec2D(130,30));
...
ol::Rect PasteTriTo( ol::Vec2D(xchroma-150,(ychroma+300)),ol::Vec2D(130,30));
...
ol::Rect RedBox(ol::Vec2D(xchroma-130,ychroma+165),ol::Vec2D(50,20));
RedBox.SetRoundness(10);
RedBox.SetLineWidth(epaisseur_ligne_fader);
RedBox.Draw(ol::Rgba::RED);
RedBox.DrawOutline(CouleurLigne);

ol::Rect GreenBox(ol::Vec2D(xchroma-65,ychroma+165),ol::Vec2D(50,20));
GreenBox.SetRoundness(10);
GreenBox.SetLineWidth(epaisseur_ligne_fader);
GreenBox.Draw(ol::Rgba::GREEN.WithAlpha(0.7));
GreenBox.DrawOutline(CouleurLigne);

ol::Rect BlueBox(ol::Vec2D(xchroma,ychroma+165),ol::Vec2D(50,20));
BlueBox.SetRoundness(10);
BlueBox.SetLineWidth(epaisseur_ligne_fader);
BlueBox.Draw(ol::Rgba::BLUE);
BlueBox.DrawOutline(CouleurLigne);

ol::Rect YellowBox(ol::Vec2D(xchroma+65,ychroma+165),ol::Vec2D(50,20));
YellowBox.SetRoundness(10);
YellowBox.SetLineWidth(epaisseur_ligne_fader);
if (index_quadri==1){YellowBox.Draw(ol::Rgba::YELLOW);}
...
//midi out de la roue
ol::Circle BMidiOut( xchroma+140,ychroma+110, 10);//box du fader
...
ol::Line (ol::Vec2D( xchroma+97,ychroma+100),ol::Vec2D(xchroma+125,ychroma+100)).Draw(CouleurLigne);
ol::Line (ol::Vec2D(xchroma+125,ychroma+100),ol::Vec2D(xchroma+125+8,ychroma+106)).Draw(CouleurLigne);
...
{
ol::Rect DockBoxColor(ol::Vec2D(xchroma-130+(65*dock_color),ychroma+215),ol::Vec2D(50,20));
...
{
ol::Rect DockBoxColor2(ol::Vec2D(xchroma-130+(65*dock_colortwo),ychroma+255),ol::Vec2D(50,20));
