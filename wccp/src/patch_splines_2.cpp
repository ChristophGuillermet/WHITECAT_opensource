...
{
   if(n<6){
   ol::Circle(curve_nodes[n].x, curve_nodes[n].y, 6).DrawOutline(CouleurBlind);
   ...
   {
     ol::Line( ol::Vec2D( curve_nodes[n].x, ypatch_window+50 ), ol::Vec2D(curve_nodes[n].x,ypatch_window+255+50)).Draw(ol::Rgba::YELLOW);
     ol::Circle(curve_nodes[n].x, curve_nodes[n].y, 6).Draw(CouleurBlind);
...
for (int d=0; d<255;d++)
{
 ol::Point(xpatch_window+30+455+d,ypatch_window+50+curve_report[curve_selected][d] ).Draw(ol::Rgba::GREEN);
}
   return(0);
}
