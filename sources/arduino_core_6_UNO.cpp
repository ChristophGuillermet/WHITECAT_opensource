...
break;
case 17://Bang!
if(digital_data_from_arduino[p]==1 && previous_digital_data_from_arduino[p]!=digital_data_from_arduino[p]) //si appuyé
{
if(arduino_digital_function_input[p][1]>0 &&arduino_digital_function_input[p][1]<128  )      
{bang_is_sended[arduino_digital_function_input[p][1]]=0;}         
}
break;
...
if(arduino_digital_type[p]==3 )//si l'affectation est type OUTPUT   PWM
{
//CHANNEL 
...
{
temp_send[3+k]=pwm_data_to_arduino[k]+1;//+1 pour éviter caractere 0
}
else {temp_send[3+k]=255;}
}
temp_send[(3+arduino_max_digital+1)]='E';
temp_send[(3+arduino_max_digital+2)]='D';
temp_send[(3+arduino_max_digital+3)]='/';
