...
{
int ard_ch=0; int ard_level=0;
sscanf(tmp_ard,"DG/%d=%d/\n",&ard_ch,&ard_level);
if(ard_ch>=0 && ard_ch<arduino_max_digital && (ard_level==0 || ard_level==1))
{
digital_data_from_arduino[ard_ch]=ard_level; 
}              
sprintf( string_Arduino_status,">>ARDUINO : Received Digital ");                       
}

if(strncmp(tmp_ard,"AN/",3)==0)
{
int ard_ch=0; int ard_level=0;
sscanf(tmp_ard,"AN/%d=%d/\n",&ard_ch,&ard_level);
if(ard_ch>=0 && ard_ch<arduino_max_analog )
{
analog_data_from_arduino[ard_ch]=ard_level; 
}              
sprintf( string_Arduino_status,">>ARDUINO : Received Digital ");                                          
}    
return(0);   
