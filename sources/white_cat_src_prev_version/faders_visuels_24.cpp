...
//SET STOP POSITION
Rect SetStop(Vec2D(x+(cmptfader*espacement)-6, y+320),Vec2D( 25,25 )); 
SetStop.SetRoundness(5);
SetStop.SetLineWidth(epaisseur_ligne_fader);




//flash si affectation
if(do_light_setpos[cmptfader]==1){SetStop.Draw(CouleurBlind);do_light_setpos[cmptfader]=0;}

//ON OFF
switch(ActionnateStopOn[cmptfader])
{
case 0:
SetStop.DrawOutline(CouleurLigne);     
break;
case 1:
SetStop.Draw(CouleurSurvol);
break;
}
...
//LFOS BUTTONS
Rect LFOButtonUp(Vec2D(x+(cmptfader*espacement)+105, y+240),Vec2D( 25,25 )); 
LFOButtonUp.SetRoundness(5);
LFOButtonUp.SetLineWidth(epaisseur_ligne_fader);
Rect LFOButtonDown(Vec2D(x+(cmptfader*espacement)+105, y+280),Vec2D( 25,25) ); 
LFOButtonDown.SetRoundness(5);
LFOButtonDown.SetLineWidth(epaisseur_ligne_fader);
Rect LFOButtonCycleUp(Vec2D(x+(cmptfader*espacement)+105, y+320),Vec2D(25,25) ); 
LFOButtonCycleUp.SetRoundness(5);
LFOButtonCycleUp.SetLineWidth(epaisseur_ligne_fader);

LFOButtonUp.DrawOutline(CouleurLigne);
LFOButtonDown.DrawOutline(CouleurLigne);
LFOButtonCycleUp.DrawOutline(CouleurLigne);
...
{
case 1:                                                 
LFOButtonUp.Draw(CouleurFader); 
break;
case 2:
LFOButtonDown.Draw(CouleurFader);
break;
default:
break;
}

LFOButtonCycleUp.Draw(CouleurFader.WithAlpha(lfo_cycle_is_on[cmptfader]));
...
Rect LFONextStepBackward(Vec2D(x+(cmptfader*espacement)+70, y+280),Vec2D( 25,25) ); 
LFONextStepBackward.SetRoundness(5);
LFONextStepBackward.SetLineWidth(epaisseur_ligne_fader);
Rect LFONextStepForward(Vec2D(x+(cmptfader*espacement)+70, y+320),Vec2D( 25,25) ); 
LFONextStepForward.SetRoundness(5);
LFONextStepForward.SetLineWidth(epaisseur_ligne_fader);
Rect LFOLoopStep(Vec2D(x+(cmptfader*espacement)+35, y+320),Vec2D(25,25) ); 
LFOLoopStep.SetRoundness(5);
LFOLoopStep.SetLineWidth(epaisseur_ligne_fader);

LFOLoopStep.DrawOutline(CouleurLigne);
LFONextStepBackward.DrawOutline(CouleurLigne);
LFONextStepForward.DrawOutline(CouleurLigne);



LFONextStepBackward.Draw(CouleurFader.WithAlpha(lfo_do_next_step[cmptfader][0]));


LFONextStepForward.Draw(CouleurFader.WithAlpha(lfo_do_next_step[cmptfader][1]));


LFOLoopStep.Draw(CouleurFader.WithAlpha(lfo_cycle_steps[cmptfader]));
...
Rect LFOButtonLoop(Vec2D(x+(cmptfader*espacement)+138, y+240),Vec2D(25,25) ); 
LFOButtonLoop.SetRoundness(5);
LFOButtonLoop.SetLineWidth(epaisseur_ligne_fader);
Rect LFOButtonALLLoop(Vec2D(x+(cmptfader*espacement)+138, y+280),Vec2D(25,25) ); 
LFOButtonALLLoop.SetRoundness(5);
LFOButtonALLLoop.SetLineWidth(epaisseur_ligne_fader);
Rect LFOButtonFlash(Vec2D(x+(cmptfader*espacement)+138, y+320),Vec2D(25,25) ); 
LFOButtonFlash.SetRoundness(5);
LFOButtonFlash.SetLineWidth(epaisseur_ligne_fader);


if(FaderIsFlash[cmptfader]==1){LFOButtonFlash.Draw(CouleurFader);}


LFOButtonLoop.DrawOutline(CouleurLigne);
LFOButtonALLLoop.DrawOutline(CouleurLigne);
LFOButtonFlash.DrawOutline(CouleurLigne);
...
//Flash
if(mouse_x>x+(cmptfader*espacement)+138 && mouse_x<x+(cmptfader*espacement)+153
...
{SetStop.Draw(CouleurBlind.WithAlpha(alpha_blinker));}
...
for (int u=0;u<8;u++)
{
 Rect Commande(Vec2D(xf,yf+(50*u)),Vec2D(72,25));
...
sprintf(temp_dex,"     off"); 
break;
case 2://minus
Mode_buffer_data.Draw(CouleurRougePur); 
sprintf(temp_dex,"subtract");
break;
case 3://add
Mode_buffer_data.Draw(CouleurLigne); 
sprintf(temp_dex,"    add");
break;
case 4://screen
Mode_buffer_data.Draw(CouleurLevel); 
sprintf(temp_dex,"  screen");
break;
case 5://exclusion
Mode_buffer_data.Draw(CouleurBleuProcedure); 
sprintf(temp_dex," exclusion");
break;
default:
break;                                          
}
Mode_buffer_data.DrawOutline(CouleurLigne.WithAlpha(0.3));
petitpetitchiffre.Print(temp_dex,x+(cmptfader*espacement)+70, y-10);

if(window_focus_id==906 && mouse_x>x+(cmptfader*espacement)+75 && mouse_x<x+(cmptfader*espacement)+115 && 
mouse_y> y-30 && mouse_y<y-20 && Midi_Faders_Affectation_Type!=0)
{  
char temp_str_tr[72];
...
if (DockTypeIs[cmptfader][(dock_used_by_fader_is[cmptfader])]==11)//Fx
 {
 //FaderNiveau.Draw(CouleurGreen);  
  switch( chaser_is_playing[ChaserAffectedToDck[cmptfader][dock_used_by_fader_is[cmptfader]]])
 {
 case 0:
 FaderNiveau.Draw(CouleurGreen);
 break;
...
else if(DockTypeIs[cmptfader][(dock_used_by_fader_is[cmptfader])]==12)//Grid
 {
 //FaderNiveau.Draw(CouleurGreen);  
 switch( grider_is_playing[faders_dock_grid_affectation[cmptfader][dock_used_by_fader_is[cmptfader]]])
 {
 case 0:
...
else if(DockTypeIs[cmptfader][(dock_used_by_fader_is[cmptfader])]==13)//Fgroup
 {
  FaderNiveau.Draw(CouleurYellowFgroup);
 
 }
 else if(DockTypeIs[cmptfader][(dock_used_by_fader_is[cmptfader])]==14)//mover
 {
  FaderNiveau.Draw( CouleurSelection);
 }
 else if(DockTypeIs[cmptfader][(dock_used_by_fader_is[cmptfader])]==15)//draw
 {      
  FaderNiveau.Draw(CouleurLevel);      
 }
  else if(DockTypeIs[cmptfader][(dock_used_by_fader_is[cmptfader])]==16)//echo
 {      
...
if(mouse_x>x+(cmptfader*espacement)+135 && mouse_x<x+(cmptfader*espacement)+150 && 
mouse_y>y-35 && mouse_y<y-20 && Midi_Faders_Affectation_Type!=0)
{  
...
for (int dd=0;dd<core_user_define_nb_docks;dd++)
{
Rect Dock(Vec2D(x+((cmptfader*espacement)+70), y + (dd*40)), Vec2D ( 50,20));
Dock.SetRoundness(8);
...
if(mouse_x>=(x+(cmptfader*espacement)+70) && mouse_x<=(x+(cmptfader*espacement)+120) 
...
//survol mode midi
if(Midi_Faders_Affectation_Type!=0 
&& mouse_x>x+(cmptfader*espacement) && mouse_x<x+(cmptfader*espacement)+40
...
if(mouse_y>(y+280) && mouse_y<(y+300) && mouse_x>x+(cmptfader*espacement) && mouse_x<(x+40+(cmptfader*espacement)))
{
